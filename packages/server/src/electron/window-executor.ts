import type { CdpSessionDomain, CdpSessionEnsureDomainOptions } from "@shuvgeist/driver/cdp-session";
import type {
...
import type { BridgeSkillSnapshotStatus } from "@shuvgeist/protocol/skill-snapshot";
...
export interface ElectronPageAssertScope {
...
export async function assertElectronWindow(
	...
	do {
		...
		if (lastResult.ok) {
			return buildElectronPageAssertResult(params, scope, true, attempts, startedAt, timeoutMs, lastResult);
		}
...
const ELECTRON_PAGE_ASSERT_SCRIPT = String.raw`function electronPageAssertScript(params) {
	...
	if (params.kind === "role") {
		...
		const hasExplicitRole = (element) => ((element.getAttribute("role") || "").trim().length > 0);
		const native = (
			role === "button"
				...
				: role === "link"
					...
					: role === "textbox"
						? Array.from(document.querySelectorAll("input:not([type]),input[type='text'],input[type='email'],input[type='search'],textarea,[contenteditable]")).filter((element) => {
