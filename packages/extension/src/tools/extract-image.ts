...
export const EXTRACT_IMAGE_DESCRIPTION = `Extract images from the current page. Returns image data that you can see and analyze.
...
export const extractImageSchema = Type.Object({
...
export interface ExtractImageSourceInfo {
	src: string;
	width: number;
	height: number;
}

export interface ExtractImageScreenshotSource {
	src: string;
	cssWidth: number;
	cssHeight: number;
	devicePixelRatio: number;
...
export interface ExtractImageToolOptions {
	windowId?: number;
	resolveSelector?: (selector: string, toolCallId: string, signal?: AbortSignal) => Promise<ExtractImageSourceInfo>;
	captureScreenshot?: (toolCallId: string, signal?: AbortSignal) => Promise<ExtractImageScreenshotSource>;
}
 ...
 */
export async function getImageInfoFromPage(tabId: number, selector: string): Promise<ExtractImageSourceInfo | string> {
	const code = `(async () => {
...
async function captureScreenshotSource(windowId: number): Promise<ExtractImageScreenshotSource> {
	const { tabId } = await resolveTabTarget({ windowId });
	const dataUrl = await chrome.tabs.captureVisibleTab(windowId, { format: "png" });
	const bitmap = await createImageBitmap(dataUrlToBlob(dataUrl));
	const viewport = await getViewportMetadata(tabId, bitmap.width, bitmap.height);
	bitmap.close?.();
	return {
		src: dataUrl,
...
export class ExtractImageTool implements AgentTool<typeof extractImageSchema, ExtractImageDetails> {
	...
	windowId?: number;
	private readonly resolveSelector?: ExtractImageToolOptions["resolveSelector"];
	private readonly captureScreenshotOverride?: ExtractImageToolOptions["captureScreenshot"];

	constructor(options: ExtractImageToolOptions = {}) {
		this.windowId = options.windowId;
		this.resolveSelector = options.resolveSelector;
		this.captureScreenshotOverride = options.captureScreenshot;
	}
	...
	async execute(
		...
		if (args.mode === "screenshot") {
			let source: ExtractImageScreenshotSource;
			if (this.captureScreenshotOverride) {
				source = await this.captureScreenshotOverride(toolCallId, signal);
			} else {
				if (!this.windowId) throw new Error("windowId not set on ExtractImageTool");
				source = await captureScreenshotSource(this.windowId);
			}
			const screenshot = await fetchAndResizeImage(source.src, maxWidth, signal);
			details.screenshot = {
				imageWidth: screenshot.imageWidth,
				imageHeight: screenshot.imageHeight,
				cssWidth: source.cssWidth,
				cssHeight: source.cssHeight,
				devicePixelRatio: source.devicePixelRatio,
				scale: screenshot.imageWidth / source.cssWidth,
			};
			...
			let info: ExtractImageSourceInfo;
			if (this.resolveSelector) {
				info = await this.resolveSelector(args.selector, toolCallId, signal);
			} else {
				const { tabId } = await resolveTabTarget({ windowId: this.windowId });
				const resolved = await getImageInfoFromPage(tabId, args.selector);
				if (typeof resolved === "string") throw new Error(resolved);
				info = resolved;
			}

			const image = await fetchAndResizeImage(info.src, maxWidth, signal);
