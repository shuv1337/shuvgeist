import type { StorageBackend, StoreConfig } from "@shuv1337/pi-web-ui/storage/types.js";
...
export type DailyCostAggregate = {
	...
	/** Durable idempotency keys for offscreen runtime message_end accounting. */
...
export class CostStore {
	...
	async recordCost(provider: string, modelId: string, cost: number, eventId?: string): Promise<void> {
		if (eventId !== undefined && !eventId.trim()) throw new Error("eventId must be non-empty when provided");
		...
		await this.backend.transaction([this.storeName], "readwrite", async (tx) => {
			...
			if (eventId) {
				aggregate.eventIds ??= {};
				if (aggregate.eventIds[eventId]) return;
				aggregate.eventIds[eventId] = true;
			}
