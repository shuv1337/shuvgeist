...
export async function readBridgeSettings(
	storage: BridgeSettingsStorageAdapter = createChromeStorageBridgeSettingsAdapter(),
): Promise<BridgeSettings> {
	return normalizeBridgeSettings(await storage.getLocalSettings());
}

export function writeBridgeSettings(
	settings: BridgeSettings,
	storage: BridgeSettingsStorageAdapter = createChromeStorageBridgeSettingsAdapter(),
): Promise<void> {
	return storage.setLocalSettings(normalizeBridgeSettings(settings));
}

export function bridgeSettingsFromStorageChange(
	changes: Record<string, chrome.storage.StorageChange>,
	areaName: string,
): BridgeSettings | null {
	if (areaName !== "local" || !changes[BRIDGE_SETTINGS_KEY]) return null;
	return normalizeBridgeSettings(changes[BRIDGE_SETTINGS_KEY].newValue as Partial<BridgeSettings> | undefined);
}
