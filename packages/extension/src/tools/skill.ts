...
export interface SkillToolOptions {
	resolveCurrentUrl?: (signal?: AbortSignal) => Promise<string>;
}

export function createSkillTool(
	options: SkillToolOptions = {},
): AgentTool<typeof skillParamsSchema, SkillResultDetails> {
	return {
		label: "Skill Management",
		name: "skill",
		description: SKILL_TOOL_DESCRIPTION,
		parameters: skillParamsSchema,
		execute: async (_toolCallId: string, args: SkillParams, signal?: AbortSignal) => {
			const skillsRepo = getSkills();
			const currentUrl = options.resolveCurrentUrl
				? await options.resolveCurrentUrl(signal)
				: ((await resolveTabTarget({ windowId: skillToolWindowId })).tab?.url ?? "");
			...
			switch (args.action) {
				...
				default:
					throw new Error(`Unknown action: ${String((args as { action?: unknown }).action)}`);
			}
		},
	};
}
