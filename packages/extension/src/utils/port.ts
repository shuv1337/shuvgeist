...
import { sidepanelTrackingPortName } from "../agent/sidepanel-context-identity.js";
...
let currentDocumentNonce: string | undefined;
...
export function initialize(
	...
	// Validate the complete identity before retaining it for reconnects.
	sidepanelTrackingPortName({ windowId, documentNonce, continuationToken, transactionId, leaseId });
	currentWindowId = windowId;
	currentDocumentNonce = documentNonce;
...
function scheduleReconnect(): void {
	if (currentWindowId === undefined || port || reconnectTimer !== undefined) return;
	reconnectTimer = setTimeout(() => {
		reconnectTimer = undefined;
		if (currentWindowId === undefined || port) return;
		try {
			port = connect();
		} catch (error) {
			console.warn("[Port] Automatic reconnect failed, retrying", error);
			scheduleReconnect();
		}
	}, 50);
}
...
export function connect(): chrome.runtime.Port {
	if (
		...
		throw new Error("[Port] Cannot connect: sidepanel identity not initialized");
	}
	if (port) return port;

	console.log(`[Port] Connecting... (${new Date().toISOString()})`);
	const tmpPort = chrome.runtime.connect({
		name: sidepanelTrackingPortName({
