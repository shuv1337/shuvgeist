...
export async function resolveTabTarget(options: BrowserTargetResolverOptions = {}): Promise<ResolvedTabTarget> {
	if (typeof options.tabId === "number") {
		...
		if (isUsableWindowId(options.windowId) && tab.windowId !== options.windowId) {
			throw new Error(
				`Tab ${options.tabId} belongs to window ${String(tab.windowId)}, not authorized window ${options.windowId}`,
			);
		}
		return {
			tabId: tab.id,
