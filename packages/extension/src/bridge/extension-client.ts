...
import { bridgeLog, type LogFields } from "@shuvgeist/protocol/logging";
import type {
	AbortMessage,
	BridgeCapability,
	BridgeRequest,
...
export class BridgeClient {
	...
	private handleMessage(msg: Record<string, unknown>): void {
		...
		if (typeof msg.id === "number" && typeof msg.method === "string") {
			if (!isBridgeSchemaMethod(msg.method)) {
			...
			const params = validateBridgeCommandParams(msg.method, msg.params);
			if (!params.ok) {
				this.sendResponse(msg.id, undefined, {
					code: ErrorCodes.INVALID_PARAMS,
					message: `Invalid parameters for '${msg.method}': ${formatBridgeCommandValidationErrors(params.errors)}`,
				});
				return;
			}
			const req: BridgeRequest = {
				...
				method: msg.method,
				params: params.value,
				...(msg.target && typeof msg.target === "object" ? { target: msg.target as BridgeRequest["target"] } : {}),
				...(typeof msg.traceparent === "string" ? { traceparent: msg.traceparent } : {}),
				...(typeof msg.tracestate === "string" ? { tracestate: msg.tracestate } : {}),
			};
	...
	private async handleRequest(req: BridgeRequest): Promise<void> {
		...
		try {
			...
			const validatedResult = validateBridgeCommandResult(req.method, result);
			if (!validatedResult.ok) {
				throw Object.assign(
					new Error(
						`Invalid result for '${req.method}': ${formatBridgeCommandValidationErrors(validatedResult.errors)}`,
					),
			...
			span?.end("ok");
			this.sendResponse(req.id, validatedResult.value);
		} catch (err: unknown) {
