...
import { type SandboxFile, SandboxIframe, type SandboxResult } from "@shuv1337/pi-web-ui/components/SandboxedIframe.js";
import type { SandboxRuntimeProvider } from "@shuv1337/pi-web-ui/sandbox/SandboxRuntimeProvider.js";

export interface ReplActivityOverlayController {
	show(taskName: string, signal?: AbortSignal): Promise<void>;
	hide(signal?: AbortSignal): Promise<void>;
}
...
export async function executeJavaScript(
	...
	taskName?: string,
	overlayController?: ReplActivityOverlayController,
	...
	if (usesBrowserjs && overlayController) {
		try {
			await overlayController.show(taskName || "Executing JavaScript", signal);
			overlayShown = true;
		} catch (error) {
	...
	const hideOverlay = async (): Promise<void> => {
		if (!overlayShown || !overlayController) return;
		overlayShown = false;
		try {
			await overlayController.hide(signal);
		} catch (error) {
			console.warn("[REPL] Failed to remove overlay:", error);
		}
	};
