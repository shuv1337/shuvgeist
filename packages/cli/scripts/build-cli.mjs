...
await build({
	...
	entryPoints: {
		shuvgeist: join(cliRoot, "src/cli.ts"),
		"direct-cdp-runtime": join(cliRoot, "src/headless/direct-cdp-runtime.ts"),
		...(includeInjectedArtifactTestSurface
			? {
					"driver-injected-artifacts": join(packageRoot, "tests/e2e/fixtures/driver-injected-artifact-surface.ts"),
				}
			: {}),
	},
	bundle: true,
...
console.log(`CLI built to ${outDir}/shuvgeist.mjs; direct-CDP runtime built to ${outDir}/direct-cdp-runtime.mjs`);
