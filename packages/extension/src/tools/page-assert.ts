import type { PageAssertParams, PageAssertResult, ResolvedPageTarget } from "@shuvgeist/protocol/protocol";
...
export interface ChromePageAssertScope {
	target: Extract<ResolvedPageTarget, { kind: "chrome-tab" }>;
...
export async function runPageAssert(
	...
	do {
		...
		attempts += 1;
		const execution = await executePageFunction<PageAssertCheckResult>(
			{ tabId: scope.tabId, frameId: scope.frameId },
			pageAssertInPage,
			{
		...
		if (lastResult?.ok) {
			return buildPageAssertResult(params, scope, true, attempts, startedAt, timeoutMs, lastResult);
		}
...
function pageAssertInPage(params: PageAssertParams): PageAssertCheckResult {
	...
	if (params.kind === "role") {
		...
		const hasExplicitRole = (element: Element): boolean => (element.getAttribute("role") ?? "").trim().length > 0;
		const native = (
			role === "button"
				? Array.from(
					...
					: role === "textbox"
						? Array.from(
								document.querySelectorAll(
									"input:not([type]),input[type='text'],input[type='email'],input[type='search'],textarea,[contenteditable]",
								),
							).filter((element) => {
						...
						: []
		).filter((element) => role !== "textbox" || !hasExplicitRole(element));
	...
	const unsupported = params as { kind: unknown };
	return { ok: false, message: `Unsupported assertion kind '${String(unsupported.kind)}'` };
}
