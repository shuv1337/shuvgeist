...
export type SnapshotBoundingBox = SnapshotInjectionBoundingBox;
...
export interface PageSnapshotResult {
	...
	truncated: boolean;
	omissions?: SnapshotOmissionMetadata;
	entries: PageSnapshotEntry[];
}

export type SnapshotOmissionMetadata = SnapshotInjectionOmissions;
...
export type SnapshotScriptEntry = SnapshotInjectionEntry;
export type SnapshotScriptResult = SnapshotInjectionResult;
export type SnapshotScriptResponse = SnapshotInjectionResponse;
export type SnapshotScriptConfig = SnapshotInjectionConfig;
...
export function formatSnapshotOmissionSummary(snapshot: PageSnapshotResult): string {
...
export async function capturePageSnapshot(options: CapturePageSnapshotOptions): Promise<PageSnapshotResult> {
	...
	const execution = await executePageFunction<SnapshotScriptResponse>(
		...
		// chrome.scripting cannot accept source text. Keep the typed source function
		// only as a compatibility fallback when userScripts is unavailable; all
		// source-text transports use the compiled artifact above.
		{ worldId: SNAPSHOT_WORLD_ID, args: [config], scriptingFallback: shuvgeistSnapshotPageScript },
	);
	...
	const snapshot = {
		...
		...(response.result.omissions ? { omissions: response.result.omissions } : {}),
	...
	async execute(
		...
		return {
			content: [
				{
					type: "text",
					text: `Snapshot captured for tab ${result.tabId}, frame ${result.frameId}; ${formatSnapshotOmissionSummary(result)}`,
				},
			],
			details: result,
		};
