...
export class BridgeTab extends SettingsTab {
	...
	private readonly storageChangeListener = (
		...
		const changedSettings = bridgeSettingsFromStorageChange(changes, areaName);
		if (changedSettings) this.applySettings(changedSettings);
	...
	private async loadSettings() {
		this.applySettings(await readBridgeSettings());
	}

	private async loadBridgeState() {
		this.applyBridgeState(await readBridgeRuntimeState(BRIDGE_RUNTIME_STATE_KEYS.bridge));
	}

	private async loadOtelState() {
		this.applyOtelState(await readBridgeRuntimeState(BRIDGE_RUNTIME_STATE_KEYS.observability));
	}
