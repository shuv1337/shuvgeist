import type { ReplOverlayAbortIntent, ReplOverlayCommand } from "@shuvgeist/driver/injected-contracts";
import { buildInjectedArtifactInvocation } from "@shuvgeist/driver/injected-invocation";
import { REPL_OVERLAY_INJECTED_ARTIFACT } from "../../injected/extension-artifacts.generated.js";
...
export async function injectOverlay(
	tabId: number,
	taskName: string,
	abortIntent: ReplOverlayAbortIntent,
): Promise<void> {
	try {
		...
		if (chrome.userScripts && typeof chrome.userScripts.execute === "function") {
			...
			const command: ReplOverlayCommand = { action: "show", taskName, abortIntent };
			await chrome.userScripts.execute({
				js: [
					{
						code: buildInjectedArtifactInvocation(REPL_OVERLAY_INJECTED_ARTIFACT, [JSON.stringify(command)]),
					},
				],
...
export async function removeOverlay(tabId: number): Promise<void> {
	try {
		if (chrome.userScripts && typeof chrome.userScripts.execute === "function") {
			const command: ReplOverlayCommand = { action: "remove" };
			await chrome.userScripts.execute({
				js: [
					{
						code: buildInjectedArtifactInvocation(REPL_OVERLAY_INJECTED_ARTIFACT, [JSON.stringify(command)]),
 ...
 */
export async function injectOverlayForActiveTab(
	taskName: string,
	abortIntent: ReplOverlayAbortIntent,
	windowId?: number,
	tabId?: number,
	...
	await injectOverlay(targetTabId, taskName, abortIntent);
	return targetTabId;
}
