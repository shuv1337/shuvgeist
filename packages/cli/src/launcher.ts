...
export interface LaunchDependencies {
	/** Shared discovery dependencies, including an injectable NodeConfigOwner. */
...
export async function launchBrowser(
	...
	// Use one owner for both lookups so browser and extension discovery honor the
	// same custom config paths and fail-closed parsing policy. Public callers can
	// inject an owner for deterministic tests or alternate runtimes.
	const discoveryDependencies: DiscoveryDependencies = {
		...dependencies.discovery,
		configOwner: dependencies.discovery?.configOwner ?? createNodeConfigOwner(),
	};

	// 1. Discover browser
	const browser = discoverBrowser(options.browser, discoveryDependencies);
	if (!browser) {
		throw new Error(browserNotFoundMessage());
	}

	// 2. Discover extension
	const extension = discoverExtensionPath(options.extensionPath, discoveryDependencies);
	if (!extension) {
