...
import { loadDeveloperSettings, updateDeveloperSettings } from "./storage/developer-settings.js";
...
const renderDebugPage = async () => {
	...
	const stored = await loadDeveloperSettings();
	let debuggerMode = stored.debuggerMode;
	let showJsonMode = stored.showJsonMode;

	const updateDebuggerMode = async (enabled: boolean) => {
		debuggerMode = enabled;
		await updateDeveloperSettings({ debuggerMode: enabled });
		renderDebugPage(); // Re-render to update UI
	};

	const updateShowJsonMode = async (enabled: boolean) => {
		showJsonMode = enabled;
		await updateDeveloperSettings({ showJsonMode: enabled });
		renderDebugPage(); // Re-render to update UI
	};
