...
interface QueryPartitionMetadata {
	queryFiltered?: number;
}

export function filterSnapshotByKeywords<
	TEntry extends SnapshotFilterEntry,
	TSnapshot extends { entries: TEntry[]; omissions?: QueryPartitionMetadata },
>(snapshot: TSnapshot, options: SnapshotFilterOptions = {}): TSnapshot {
	const query = options.query?.trim();
	if (!query) {
		return { ...snapshot, entries: [...snapshot.entries] };
	}
	// Current snapshot producers apply the query as a strict eligibility partition
	// before budgeting. A second, differently-scored filter here would discard
	// already-accounted entries and make omission metadata inconsistent. Keep the
	// legacy filter only for snapshots that do not carry the partition count.
	if (typeof snapshot.omissions?.queryFiltered === "number") {
		return { ...snapshot, entries: [...snapshot.entries] };
	}
