...
export class ElectronTargetsTab extends SettingsTab {
	...
	private readonly storageChangeListener = (
		...
		if (areaName === "session" && changes[BRIDGE_ELECTRON_STATE_KEY]) {
			this.state = changes[BRIDGE_ELECTRON_STATE_KEY].newValue as BridgeElectronStateData;
	...
	private async loadState() {
		const result = await chrome.storage.session.get(BRIDGE_ELECTRON_STATE_KEY);
		this.state = (result[BRIDGE_ELECTRON_STATE_KEY] as BridgeElectronStateData | undefined) ?? {
			sessions: [],
			updatedAt: "",
		};
	}
	...
	private async refreshFromBridge() {
		try {
			...
			await chrome.storage.session.set({ [BRIDGE_ELECTRON_STATE_KEY]: this.state });
			this.updateThumbnailRefresh();
		} catch (error) {
			Toast.error(`Failed to refresh Electron targets: ${(error as Error).message}`);
		}
	}
